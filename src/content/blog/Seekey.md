...
- 输入密码等敏感内容时，可以改用固定的 `<若干字符>` 标签，或完全隐藏普通字符而保留快捷键；
...
打开 GUI 或 TUI 时，屏幕上始终只保留一个按键渲染面：真实输入浮层已经运行时继续显示它，不再叠加样例；没有主浮层时才创建会跟随内存配置变化的实时样例。调整按键尺寸、字体、背景、边框宽度、圆角、对齐方式、显示时长或者渐隐速度时，可以直接观察当前唯一浮层。Seekey 当前提供七套内置主题、自定义图标和 `@matugen:<role>` 颜色引用，可以让气泡跟随壁纸配色。各字段和配置文件优先级见[配置与编辑器](/notes/seekey/configuration)。

演示中需要输入密码时，`typing-display=masked` 会让一整段连续输入始终只显示一个固定标签，不暴露内容或字符数量；`typing-display=off` 则不显示普通字符，`Ctrl+C` 等组合键与非文字按键仍然可见。
