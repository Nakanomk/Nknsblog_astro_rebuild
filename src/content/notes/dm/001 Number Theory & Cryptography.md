---
title: 'DM Number Theory & Cryptography'
...
$$
a\equiv b \pmod{m} \Rarr b\equiv a\pmod{m}
$$
...
- **加法逆元** 如果 
  ...
  $$
   ...
   ，那么 m - a 是 a 的 模 m 加法逆元.0 是它自身的模 m 加法逆元.
...
**乘法逆元在模 m 的运算中并不总是存在** 例如，2 在模 6 的情况下没有乘法逆元，因为没有整数 x 使得  
...
$$
a = p_1^{r_1}p_2^{r_2}p_3^{r_3}...p_k^{r_k}
$$
...
$$
 ...
 是正整数，则正整数 d 为 a 的因子的充份必要条件是 
$$
d = p_1^{s_1}p_2^{s_2}...p_k^{s_k}
$$

，其中 
$$
0 \leq s_i \leq r_i, i = 1,2,...,k
$$
...
由该定理得知，随机选择一个小于 n 的正整数是素数的概率大约是 

$$
\frac{\frac{n}{\ln n}}{n} = \frac{1}{\ln n}
$$
...
假设 a 和 b 是正整数 且 a ≥ b. 设 

$$
r_0 = a, r_1 = b
$$

.通过连续应用除法算法啊，我们得到如下结果：
...
如果 a 和 b 是任意整数，且不全为 0,那么 gcd(a,b) 是集合 
...
$$
a \bmod s = a - qs =  - q (ax+by) = a(1-qx) + b(-qy)
$$
